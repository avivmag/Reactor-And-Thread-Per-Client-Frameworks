...
public class CommandParamsSeparatorTokenizer implements MessageTokenizer<CommandMessage> {
	...
	public CommandParamsSeparatorTokenizer(String separator, Charset charset) {
	...
	public synchronized boolean hasMessage() {
		// meaning there is some buffered bytes to be read
		while(_buffers.size() > 0) {
			// remove the first buffer arrived (the oldest)
	 ...
	 */
	public synchronized CommandMessage nextMessage() {
		String message = null;
		...
		// from here on, we are trying to find the appropriate command message and return it.
		String[] splitMessage = message.split(" ");
		if(splitMessage.length == 0)
			return new CommandMessage("", "");
		
		if(splitMessage.length == 1)
			return new CommandMessage(splitMessage[0], "");
		
		StringBuilder builder = new StringBuilder();
		for(int i = 1; i < splitMessage.length; i++ )
		    builder.append((i!=1 ? " " : "") + splitMessage[i]);
		
		return new CommandMessage(splitMessage[0], builder.toString());
	}
	 ...
	 */
	public ByteBuffer getBytesForMessage(CommandMessage msg)  throws CharacterCodingException {
